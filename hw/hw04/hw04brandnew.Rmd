...
ord %>% 
  drop_na()
```
>according to this code above, there are still 31,334 observations, and using drop_na() to remove them, none of them were removed, so there are no missing dates in this range.
...
```{r}
ord %>% 
  select(airport, hour, time_of_day, flights) %>% 
  group_by(airport, hour, time_of_day) %>% 
  summarize(sum_flights = sum(flights)) %>% 
  ungroup() %>% 
  slice_max(hour, n=6)
```
...
```{r}
ggplot(ord, aes(x = time_of_day, y = flights)) + 
